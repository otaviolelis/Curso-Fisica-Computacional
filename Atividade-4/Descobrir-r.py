...
num_pontos=20
torch.manual_seed(0)
t = np.linspace(0, 500, num_pontos)
...
class RedeNeural(nn.Module):
    def __init__(self):
        ...
        # r como parâmetro inicializado em 0.01 (exemplo)
        self.log_r = nn.Parameter(torch.log(torch.tensor(0.01))) 
        # ao usar log_r, garantimos r>0 via exp()
    ...
    @property
    def r(self):
        # r = exp(log_r) garante positividade
...
def physics_residual(t):
    ...
    return dT_dt - model.r * (Tamb - T_pred)
...
print(f"r estimado = {model.r.item():.5f}")
